...
def generate_xml(num_blocks):
    ...
    texture_base = (
        '<texture name="block_named" file="../textures/cube.png" gridsize="3 4" gridlayout=".U..LFRB.D.."></texture>'
    )
    asset_base = (
        '<material name="block{id}_mat" texture="block_named" specular="0" shininess="0.5" reflectance="0"></material>'
    )
    # asset_base = '<material name="block{id}_mat" texture="block_named" specular="0" shininess="0.5" reflectance="0" rgba="{color} 1"></material>'

    sites = []
    block_bodies = []
    assets = []

    assets.append(texture_base)
    # assets.append(asset_base)
